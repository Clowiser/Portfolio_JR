export const locales = {
    en: "en-US",
    fr: "fr-FR",
};
// REVOIR TOUS LES TEXTES

export const message = {
    [locales.en]: {
        //General
        loading_api_error: "We are sorry, we can't load information for the moment, try again later!",
        loading_api_loading: "Loading, please wait...",
        //Home
        home_title: "Home",
        home_header_title: "Junior Frontend Developer & Explorer of Imaginary",
        home_welcome_message: "Hello World!",
        home_reconversion_text: "Connected budding artist and frontend developer apprentice, I welcome you! Today, I am retraining in the digital world as an IT Developer. Indeed, passionate about innovation and technologies, I want to put my creative skills\n and my technical know-how at the service users and companies!",
        home_find_creation: "Please find my personal creations and my creations made within the framework of my training at the Campus Numérique in the ALPS in Chambéry, in my portfolio!",
        realisation_title: "They trusted me!",
        skill_title_imaginary: "Explorer of Imaginary",
        skill_subtitle_imaginary: "Armed with my sketches since childhood, I like to create and transfer my appetites in the digital environment",
        skill_title_list_imaginary: "Creative brushes",
        skill_title_list_pen_paper_imaginary: "Pen & paper",
        skill_title_list_stylet_tablette_imaginary: "Stylet & tablet",
        skill_title_list_figma_imaginary: "Figma",
        skill_title_list_gimp_imaginary: "Gimp",
        skill_title_list_blender_imaginary: "Blender",
        skill_title_list_photoshop_imaginary: "Photoshop",
        skill_title_developer: "Junior Frontend Developer",
        skill_subtitle_developer: "Developing block by block my skills, I have fun in a precise way",
        skill_title_list_developer: "Development tools",
        skill_title_html_css_developer: "HTML & CSS & JS",
        skill_title_react_developer: "React",
        skill_title_typescript_developer: "TypeScript",
        skill_title_Github_developer: "GitHub",
        //Portfolio
        portfolio_title: "Portfolio",
        portfolio_web_work_title: "My Web Work",
        portfolio_web_work_subtitle: "From project management to the creation of an application, find all of my work carried out during my training at the Digital Campus in the ALPS and outside",
        portfolio_graphic_work_title: "My Design Work",
        portfolio_graphic_illustrations_work_title: "Digital Illustrations",
        portfolio_graphic_3D_work_title: "3D Work",
        portfolio_graphic_literary_work_title: "Literary Work",
        portfolio_subtitle: "",
        //About
        about_title: 'Who I am?',
        about_presentation: "",
        about_subtitle_about: "About me!",
        about_subtitle_way: "My way",
        //Contact
        contact_title: "Contact",
        form_submit: "Send",
        //Footer
        Footer_copyright: "Copyright by me 2022",
        Footer_Motto: "Fulfill yourself, keep learning and level up",
    },
    [locales.fr]: {
        //General
        loading_api_error: "Nous sommes désolés, nous ne pouvons pas charger les informations pour le moment, réessayez plus tard !",
        loading_api_loading: "Chargement, veuillez patienter...",
        //Home
        home_title: "Accueil",
        home_header_title: "Développeuse Frontend Junior & Exploratrice de l'Imaginaire",
        home_welcome_message: "Bonjour Monde !",
        home_reconversion_text: "Artiste en herbe connectée et Apprentie développeuse d'interface, je vous souhaite la bienvenue ! En pleine reconversion dans l'univers du numérique, j'intègre mes appétences pour la création et le visuel à mon envie d'évoluer sur les technologies, ceci au service des utilisateurs et des entreprises !",
        home_find_creation: "Veuillez retrouver l'ensemble de mes créations dans mon portfolio !",
        realisation_title: "Ils m'ont fait confiance !",
        skill_title_imaginary: "Exploratrice de l'Imaginaire",
        skill_subtitle_imaginary: "Armée de mon carnet à croquis ou de ma tablette graphique, je me plais à m'évader dans l'imaginaire et de donner vie à mes idées ! ",
        skill_title_list_imaginary: "Les pinceaux de la création",
        skill_title_list_pen_paper_imaginary: "Crayon et papier",
        skill_title_list_stylet_tablette_imaginary: "Stylet et tablette",
        skill_title_list_figma_imaginary: "Figma",
        skill_title_list_gimp_imaginary: "Gimp",
        skill_title_list_blender_imaginary: "Blender",
        skill_title_list_photoshop_imaginary: "Photoshop",
        skill_title_developer: "Développeuse Frontend Junior",
        skill_subtitle_developer: "Face au mode sombre de l'IDE, je reste passionnée et amusée par l'ensemble du processus de création d'une application ou d'un site web ! ",
        skill_title_list_developer: "Les outils du développement",
        skill_title_html_css_developer: "HTML/CSS/JS",
        skill_title_react_developer: "React",
        skill_title_typescript_developer: "TypeScript",
        skill_title_Github_developer: "GitHub",
        //Portfolio
        portfolio_title: "Portfolio",
        portfolio_web_work_title: "Mes travaux dans le web",
        portfolio_web_work_subtitle: "De la gestion de projet à la création d'une application, retrouvez l'ensemble de mes travaux réalisés lors de ma formation au Campus Numérique in the ALPS et aussi en dehors, afin de poursuivre ma progression et de m'amuser !",
        portfolio_graphic_work_title: "Mes travaux dans la création",
        portfolio_graphic_illustrations_work_title: "Illustrations Numériques",
        portfolio_graphic_3D_work_title: "Créations 3D",
        portfolio_graphic_literary_work_title: "Créations Littéraire",
        portfolio_subtitle: "Mes réalisations faites au Campus Numérique in the Alps de Chambéry, et en dehors !",
        //About
        about_title: 'Qui suis-je ?',
        about_subtitle_about: "A propos de moi !",
        about_subtitle_way: "Mon parcours",
        about_presentation: "",
        //Contact
        contact_title: "Contact",
        form_submit: "Envoyer",
        //Footer
        Footer_copyright: "Copyright Jessica Rigaud 2022",
        Footer_Motto: "Se réaliser, continuer d'apprendre et passer au niveau supérieur",
    }
};

export default message;